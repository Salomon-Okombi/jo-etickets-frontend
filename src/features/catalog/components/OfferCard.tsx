...
export default function OfferCard({
  ...
  // ✅ correction : utiliser est_disponible
  const outOfStock = !offer.est_disponible;

  const handleAdd = async () => {
    if (!onAddToCart) return;

    const maxQty = offer.quota_billets_restant ?? 1;
  ...
  return (
    <div
      className={`card bg-base-100 shadow-sm border border-base-200 hover:shadow-md transition ${
        className ?? ""
      }`}
    >
        ...
        <div className="card-actions items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              type="number"
              min={1}
              max={offer.quota_billets_restant ?? 1}
              value={qty}
              onChange={(e) =>
